...
module.exports = HandleMsg = async (urbae, message) => {
	try {
		...
		const botNumber = await urbae.getHostNumber() + '6287856906040@c.us'
...
// END SCRIPT IDENTIFY OWNER

	// START COMMAND REPORT BOT
	...
	// END COMMAND REPORT BOT
	
	// START AUTO REPLY CHAT BOT NON PREFIX 
	...
	// END AUTO REPLY CHAT BOT NON PREFIX 	

	// START REPLY CHAT TAG OWNER 
	...
	// END REPLY CHAT TAG OWNER
		...
		/* Input Menu Image*/
		const menupict = "https://i.ibb.co/37pMWkM/mdsid.jpg"
		...
		if (isCmd && isMuted(chatId) && banChat() && !isBlocked && !isBanned || isOwnerB || isPrem) {
			switch (command) {
				...
				case prefix + 'tnc':
					...
					break
				case prefix + 'help':
					const bots = `Hi ${pushname}, this is MDSID Bot, to find out the commands menu, type *${prefix}menu* , *${prefix}p*`
...
// START SCRIPT COMMAND LIST MENU
				...
				case prefix + 'menuowner':
					if (!isOwner) return await urbae.reply(from, 'Perintah ini hanya bisa digunakan oleh Owner!', id)
					const php4 = menupict
					await urbae.sendFileFromUrl(from, php4, 'image.jpg', menuId.owner(prefix), id)
					break					
					
				/* Command Menu Admin (Group) */	
				case prefix + 'menuadmin':
					...
					const ppt = menupict
					await urbae.sendFileFromUrl(from, ppt, 'image.jpg', menuId.admin(prefix), id)
					break

// END SCRIPT COMMAND LIST MENU
